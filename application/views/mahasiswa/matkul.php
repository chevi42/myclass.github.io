<div class="container">
  <div class="row">
    <div class="col py-3">
        <div class="card bg-primary" style="width: 60rem; height: 10rem;">
            <div class="card-body">
                <div class="row">
                    <div class="col">
                    <img src="<?php echo base_url();?>images/class.png" height="125px">
                    </div>
                    <div class="col-4">
                
                    </div>
                    <div class="col">
                        <h4 class="card-title text-white">Pemograman Web IoT</h4>
                    </div>
                </div>   
            </div>
        </div>
    </div>
  </div>
  <div class="row">
  </div>   
  <div class="row">
    <div class="col py-2">
        <div class="card" style="width: 60rem;">
            <div class="card-body">
            <div class="row">
                  <div class="col">
                    <h5 class="card-title">Materi 1</h5>
                  </div>
                </div>
                <h6 class="card-text">Silahkan lihat dan pelajari materi yang telah saya berikan</h6>
                <br>
                <div class="row">
                  <div class="col-md5">
                    <a href="#" class="card-link"><img src="<?php echo base_url();?>images/text.png" height="40px"></a>
                    <a href="#" class="card-link"><img src="<?php echo base_url();?>images/video.png" height="40px"></a>
                    <a href="#" class="card-link" style="visibility: hidden"><img src="<?php echo base_url();?>images/AR.png" height="40px"></a>
                    <a href="#" class="card-link" style="visibility: hidden"><img src="<?php echo base_url();?>images/VR.png" height="40px"></a>
                  </div>
                  <div class="col d-flex justify-content-end">
                    <input class="btn btn-outline-primary" type="button" value="Hadir" disabled></input>
                  </div>
                </div>
            </div>
        </div>
    </div>
  </div>
  <div class="row">
    <div class="col py-2">
        <div class="card" style="width: 60rem;">
            <div class="card-body">
                <div class="row">
                  <div class="col">
                    <h5 class="card-title">Materi 2</h5>
                  </div>
                </div>
                <h6 class="card-text">Silahkan pelajari rangkaian yang telah saya berikan</h6>
                <br>
                <div class="row">
                  <div class="col-md-5">
                    <a href="#" class="card-link"><img src="<?php echo base_url();?>images/text.png" height="40px"></a>
                    <a href="#" class="card-link"><img src="<?php echo base_url();?>images/video.png" height="40px"></a>
                    <a href="#" class="card-link"><img src="<?php echo base_url();?>images/AR.png" height="40px"></a>
                    <a href="#" class="card-link"><img src="<?php echo base_url();?>images/VR.png" height="40px"></a>
                  </div>
                  <div class="col d-flex justify-content-end">
                    <input class="btn btn-outline-primary" type="button" value="Hadir"></input>
                  </div>
                </div>
            </div>
        </div>
    </div>
  </div>      
</div>